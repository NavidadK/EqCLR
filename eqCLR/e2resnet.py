##### https://github.com/dmklee/equivision/blob/main/equivision/e2resnet.py #########
...
class E2BasicBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        print(f'In_type size: {in_type}, Out_type size: {out_type}')
...
class E2ResNet(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x) # hier escnn module not torch
        h = self.gpool(x).tensor.squeeze(-2).squeeze(-1)
        z = self.fc(h)
        return h, z